...
### 4. Leitor em massa (quando as tres nao bastam e o arquivo e grande)
Arquivo acima do teto (350 linhas): indice, assinatura e fatia nao bastam para ENTENDER o todo, e
ler o arquivo inteiro no modelo principal custa caro. O hook `scripts/read-shunt-guard.ps1` nega a
leitura inteira nesse caso e aponta a rota: fatia (offset/limit) para editar ou pegar trecho exato;
sub-agente em modelo barato para entender, que devolve so um resumo em bullets. Doutrina completa:
[skills/leitura-em-massa/SKILL.md](../skills/leitura-em-massa/SKILL.md).

> Ordem de leitura: indice (1) -> assinatura (2) -> fatia (3) -> leitor em massa (4) para arquivo
> acima do teto; arquivo inteiro no contexto do modelo principal so abaixo do teto. A metrica e a
> mesma do produto: custo medio por Artifact cai com o tempo (tools.md).
...
`scripts/read-shunt-guard.ps1` (hook de PreToolUse) nega a leitura inteira de arquivo acima do teto
(`ALIA_SHUNT_MIN_LINES`, padrao 350) e devolve a rota no proprio erro; Read com offset/limit segue liberado.
