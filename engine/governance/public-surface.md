...
|---|---|---|
| Produto | engine/, scripts/ do motor (allowlist), skills/, onboarding/, optional-mcps/, studio.example/, .claude/ (settings+comando), docs/README.md, docs/COMPATIBILIDADE.md, docs/INTEGRIDADE.md | scripts/smoke-test-studio.ps1 (smoke DA INSTANCIA), scripts/migrate-to-studio.ps1, scripts/extract-secrets.ps1 (migracao unica do dono), scripts/fixtures/ ship (sao fixture de teste do proprio smoke, sem segredo), benchmarks/ (TASK-624: medicao interna, nao ship) |
