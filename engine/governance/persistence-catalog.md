...
| `engine/governance/law-ledger-sem-teste-baseline.txt` | ratchet | humano (edicao manual, so quando um teste novo cobre uma LEI que estava `SEM TESTE`) | `scripts/law-ledger-check.ps1` secao (D) via `scripts/smoke-test.ps1` | so encolhe; LEI nova sem teste, ou teste que existia e saiu do smoke, sobe a contagem e reprova (TASK-511, WARDEN, 09/09/2026; baseline gravado hoje: 8) |
