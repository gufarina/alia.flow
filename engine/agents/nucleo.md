...
= usa-lo e obrigatorio; generico so quando nenhum cobre a lente. Especialista delega so ao leitor em
massa (Explore em Haiku, ate 3 por Task); o motor bloqueia qualquer outra delegacao dentro de
sub-agente. Detalhe: engine/governance/response-guard.md
...
uma vez; fonte lida nao se rele. Fonte que eu so preciso ENTENDER (nao editar) com mais de 150
linhas vai para o leitor em massa (Agent em Haiku, skills/leitura-em-massa), nunca para o meu
contexto; fatia no modelo caro so para editar ou citar trecho exato. Arquivo acima de 350 linhas:
o hook nega a leitura inteira e aponta a rota (fatia com offset/limit para editar; leitor em massa
em modelo barato para entender: skills/leitura-em-massa). Detalhe: engine/reading-strategy.md
