...
## Indice estrutural: presente em TODO Client (OPP-68); grafo semantico e enriquecimento (L67)

> LEI: o INDICE ESTRUTURAL (`squad/knowledge/edges.json`, gerado por `scripts/kb-index.ps1`, custo
> zero de modelo) e a pre-condicao operacional DECLARADA de CADA Client, nao so do demo. O Squad
> Creator o gera como passo de fechamento antes do handoff (ver `agents/squad-creator.yaml`).
> `scripts/graph-check.ps1` passa a REPROVAR squad sem `edges.json` na 2.0.0 - ATE LA ele continua
> reprovando pela falta de `graphify-out/` (MEDIDO 15/09/2026: virar agora derrubaria 6 Clients
> reais que hoje passam - quebra de contrato nao viaja em versao menor). O grafo SEMANTICO do
> graphify (`squad/knowledge/graphify-out/`: `graph.json` +
> `GRAPH_REPORT.md`) deixa de ser pre-condicao e vira ENRIQUECIMENTO EXPLICITO - criterio de quando
> pagar em L67 do law-ledger.

## Invariantes

- **Sem Squad = sem Tasks** (Principio I + III). Trabalho de Client passa pelo Squad do Client.
  Excecao unica: camada B/C pode acionar Task do leitor em massa (Explore/Haiku, ate 3, sob cerca
  do motor); nenhuma outra delegacao e permitida a folha.
- **Sem Especificacao de Entrega = sem consistencia**: todo Client tem a spec de entrega no knowledge.
- **Sem indice estrutural = squad incompleto** (declarado): todo Client tem `edges.json` gerado; `graph-check.ps1` passa a reprovar a falta dele na 2.0.0 - ate la reprova pela falta de `graphify-out/` (L67). Grafo semantico e enriquecimento sob criterio (L67), nao mais universal.
