<#
  ...
  -Mode spawn (padrao) - host com sub-agente nativo (ex: Claude Code):
    ...
    QUANDO acionar). tools = tools[] do yaml filtrados contra a lista real do Claude Code; toda
    camada ganha Task (TASK-571, L66) SOMENTE porque scripts/leitor-gate.ps1 (PreToolUse) cerca
    esse Task de folha a um unico uso: o leitor em massa (Explore/haiku/molde de
    skills/leitura-em-massa/SKILL.md); sem tools validos no yaml, cai no padrao por camada.
  ...
  "supabase" quando so existe "supabase-acme") -> corrige o namespace com aviso. Sem correspondencia
...
param(
 ...
 [string]$Only,
 # -Reserve (TASK-804, dieta de token): so vale com -Client e -Mode spawn. Em vez de gravar em
 # .claude\agents (a pasta que o host escaneia inteira), grava em .claude\squads\{client}\ - uma
 # reserva por Client que NUNCA e lida pelo host sozinha. v2\bin\client.py e quem sincroniza
 # .claude\agents a partir da reserva, so com o squad do Client ativo. Gerar para a reserva nao
 # muda quem fica visivel na sessao atual - so client.py (rodado ANTES de abrir a sessao) faz isso.
 [switch]$Reserve
)
...
if ($Reserve) {
    if ($Mode -ne 'spawn') { Write-Error "-Reserve so vale com -Mode spawn (a reserva serve o roster do Claude Code)"; exit 1 }
    if (-not $Client) { Write-Error "-Reserve exige -Client (a pasta de reserva e por Client, nunca compartilhada)"; exit 1 }
    $outDir = Join-Path $RepoRoot ".claude\squads\$Client"
}
# o gabarito de briefing (TASK-683/TASK-801 E7) NUNCA mora em .claude\agents nem .opencode\agent -
# so pasta que o host escaneia como agente invocavel entra ali; o briefing e rascunho para humano
# preencher, sibling fora da pasta que o host le (conserto medido: 164 arquivos em .claude\agents
# contra 97 no estudio vivo, coordenacao TASK-801 E7).
$briefsDir = Join-Path $RepoRoot '.claude\briefs'
...
# servidor real -> mantem. correspondencia obvia (prefixo, ex "supabase" -> "supabase-acme") ->
...
function New-BeforeActingBlock {
 ...
 # ---- TASK-804, revisao e7: o ritual fixo de leitura (MAP, client.md/grounding,
 # GRAPH_REPORT, lista de knowledge) so vale para o Gateway (Camada A) - e ele quem carrega o
 # segundo cerebro inteiro. O Specialist (B/C) nao carrega mais isso no corpo: o brief da Task
 # (v2/bin/brief.py) entrega os 6 campos do checklist + as fatias exatas (caminho#Lx-Ly) na
 # hora de abrir a Task, ~4,1 mil tokens a menos por spawn (e7-revisao-ddd-independente.md,
 # achado "Ritual fixo por spawn"). ----
 if ($Camada -eq 'A') {
 ...
 [void]$lines.Add('## Antes de agir')
 [void]$lines.Add('')
 [void]$lines.Add('O brief da Task (aberta com v2/bin/brief.py) traz os 6 campos do checklist mais as fatias exatas no formato caminho#Lx-Ly. Nao ha ritual fixo de leitura aqui - a fatia do brief e a fonte.')
}
...
foreach ($clientDir in $clientDirs) {
    ...
    foreach ($yamlFile in $agentYamls) {
        ...
        try {
            ...
            $skillsEntries = Get-ListValue $data 'skills'
 ...
 throw "falta 'authority.escalates_to' no yaml (obrigatorio em toda camada - a quem sobe e em que caso). Sugestao para camada $camada`: $($defAuthority.escalates_to)"
            ...
            # ---- description (TASK-801, E5, contrato Gate 23/09/2026: encurtada ao "acionar
            # quando" - a descricao curta ja basta pro modelo saber QUANDO invocar; role/dominio
            # seguem no corpo do agente, nao precisam duplicar aqui) ----
            ...
            # mcp__supabase__* quando o servidor real e mcp__supabase-acme__*).
            ...
            # TASK-801 (E5, contrato Gate 23/09/2026, objecao 12 do Weaver ACEITA): Agent/Task
            # (fan-out de sub-agente) so nasce em quem tem gateway: true no squad.yaml - so o
            # Squad Owner orquestra; Specialist folha nunca ganha fan-out por padrao (antes, TODA
            # camada ganhava Task so porque leitor-gate.ps1 cercava o uso ao leitor em massa -
            # TASK-571, L66; essa cerca continua valendo PARA o Gateway, que ainda so pode acionar
            # o leitor em massa e mais nada alem disso). Skill entra na lista de TODO Specialist
            # (folha ou Gateway), Gateway ou nao - objecao 11 ACEITA.
            $isGateway = [bool]($squadInfo.GatewayId -and $squadInfo.GatewayId -eq $agentId)

            if (-not $validTools -or $validTools.Count -eq 0) {
                switch ($camada) {
                    'A' { $validTools = @('Read', 'Grep', 'Glob') }
            ...
            if ($isGateway -and ($validTools -notcontains 'Task')) {
                $validTools = @($validTools) + @('Task')
            }

            if ($validTools -notcontains 'Skill') {
                $validTools = @($validTools) + @('Skill')
            }
 ...
 foreach ($bl in (New-BeforeActingBlock -Camada $camada -EntryPointPath $entryPointPath -GroundingPath $groundingPath -KnowledgeLines $knowledgeLines -BudgetToolCalls $budgetToolCalls -OutputMaxLines $outputMaxLines -EvidenceTags $evidenceTags -AuthorityDecides $authorityDecides -AuthorityEscalatesTo $authorityEscalatesTo)) { [void]$sections.Add($bl) }
                ...
                # ---- Skills declaradas (TASK-801, E5: campo opcional skills: na persona) ----
                if ($skillsEntries.Count -gt 0) {
                    [void]$sections.Add('')
                    [void]$sections.Add('## Skills declaradas')
                    [void]$sections.Add('')
                    foreach ($sk in $skillsEntries) { [void]$sections.Add("- $sk") }
                }
                ...
                if ($camada -ne 'A') {
                    ...
                    [void]$sections.Add('')
                    [void]$sections.Add('Voce pode acionar apenas o leitor em massa (Explore em Haiku, molde em skills/leitura-em-massa/SKILL.md), no maximo 3 vezes; qualquer outra delegacao e bloqueada pelo motor.')
                }
 ...
 foreach ($bl in (New-BeforeActingBlock -Camada $camada -EntryPointPath $entryPointPath -GroundingPath $groundingPath -KnowledgeLines $knowledgeLines -BudgetToolCalls $budgetToolCalls -OutputMaxLines $outputMaxLines -EvidenceTags $evidenceTags -AuthorityDecides $authorityDecides -AuthorityEscalatesTo $authorityEscalatesTo)) { [void]$sections.Add($bl) }
            ...
            $briefPath = Join-Path $briefsDir "$name.brief.md"
