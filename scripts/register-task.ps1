<#
  ...
  Projeto de um Cliente ANTES de executar, e a Task carrega a LINHAGEM do trabalho:
   ...
   - plan          : o id da Task-mae (TASK-NNN) quando esta Task e uma ETAPA de um plano
                     (TASK-585). Opcional - SEM grafo de precedencia entre etapas, so o vinculo
                     etapa->mae. Nasce so quando -Plan e informado, nunca retroativo.
 ...
 grava nada de novo (campo nasce so quando informado, nunca inventado, nunca retroativo).
  ...
  LIMITACAO ACEITA (1.80.0): nao ha transacao entre a escrita do state.json e a chamada de
  Add-Event. Se o processo morrer entre as duas, a Task fica gravada SEM o evento correspondente -
  o diario PERDE evento, mas nunca inventa um (o evento so nasce depois do estado escrito). Por
  isso o state.json segue a fonte de verdade do status e studio/events.jsonl e diario DERIVADO:
  reconciliar os dois nao e trabalho deste script.
...
# JOURNAL DE EVENTOS (TASK-579, contrato do LATTICE): appenda um evento por mudanca de estado de
# Task (criacao, mudanca de status, veredito de gate, fechamento) em arquivo SEPARADO do state.json
# (events.jsonl, irmao do -StateFile - nunca dentro do snapshot mutavel que e reescrito por
# inteiro a cada chamada). Schema (ts, task_id, step, type, ref) e o que skills/state-resume ja
# espera ler (nao inventa campo novo). Append via StreamWriter em modo Append, UTF-8 sem BOM, uma
# linha JSON por evento - sem lock explicito (FileMode.Append cobre a maioria dos casos de
# concorrencia, mesmo raciocinio do incidente de ID de Task colidido).
$eventsFile = Join-Path $studioRoot "events.jsonl"
function Add-Event {
  param(
  ...
  $evTs = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
  $ev = [ordered]@{ ts = $evTs; task_id = $TaskId; step = $Step; type = $Type; ref = $Ref }
  $line = ($ev | ConvertTo-Json -Compress)
  $sw = New-Object System.IO.StreamWriter($EventsFile, $true, $utf8)
...
if (-not [string]::IsNullOrWhiteSpace($Id)) {
  ...
  $effPlan        = if ($PSBoundParameters.ContainsKey('Plan'))        { $Plan }          else { "$($task.plan)" }
  ...
  # CUSTO AUTOMATICO NO FECHA (TASK-569): antes travava exigindo -Tokens/-ToolUses manuais; agora
  # mede sozinho no transcript da sessao (Get-TranscriptUsage, scripts/_studio.ps1) quando o
  # operador nao informou os dois. So volta a travar se -Tokens foi passado NEGATIVO DE PROPOSITO
  # (sinal explicito do operador pra reprovar o fechamento sem custo).
  $tokensSourceEff = ""
  if (($effStatus -eq "done" -or $effStatus -eq "review") -and $effSpecialist -ne "alia") {
    if ($PSBoundParameters.ContainsKey('Tokens') -and $Tokens -lt 0) {
      Write-Host ("[ERRO] Task " + $effStatus + " de '" + $effSpecialist + "' com -Tokens negativo de proposito: custo e obrigatorio no fecha. Omita -Tokens/-ToolUses pra medir automatico do transcript, ou informe os dois valores.")
      exit 1
    }
    if ($effTokens -lt 0 -or $effToolUses -lt 0) {
      $auto = Get-TranscriptUsage -SessionId $effSession
      if ($auto.source -eq "auto-transcript") {
        if ($effTokens -lt 0) { $effTokens = $auto.tokens }
        if ($effToolUses -lt 0) { $effToolUses = $auto.tool_uses }
        $tokensSourceEff = "auto-transcript"
      } else {
        $tokensSourceEff = "indisponivel"
      }
    }
  }
  ...
  foreach ($pn in @("title","status","specialist","agent_id","type","artifact","base_artifact","plan","session","gate_verdict","operator_order")) {
  ...
  foreach ($propName in @("tokens","tool_uses","budget","budget_exceeded","tokens_source","title_history","updated","executed_by")) {
  ...
  $task.tokens_source   = $(if ($tokensSourceEff -ne "") { $tokensSourceEff } else { $null })
  ...
  # Ordem importa pro resume (state-resume le o evento mais recente por Task): gate ANTES de
  # fechamento, pra "done" ficar por ultimo quando os dois disparam na mesma chamada (fecha com
  # veredito).
  if ($PSBoundParameters.ContainsKey('GateVerdict') -and -not [string]::IsNullOrWhiteSpace($effGate) -and $effGate -ne $oldGate) {
    Add-Event -EventsFile $eventsFile -TaskId $Id -Step "MONITORA" -Type "gate" -Ref $effGate
  }
  if ($effStatus -ne $oldStatus) {
    if ($effStatus -eq "done") {
      Add-Event -EventsFile $eventsFile -TaskId $Id -Step "FECHA" -Type "done" -Ref $(if (-not [string]::IsNullOrWhiteSpace($effArtifact)) { $effArtifact } else { $Id })
    } else {
      Add-Event -EventsFile $eventsFile -TaskId $Id -Step "DELEGA" -Type "delegated" -Ref $effStatus
    }
  }
...
# (sinal explicito do operador pra reprovar o fechamento sem custo).
$tokensSource = ""
if (($Status -eq "done" -or $Status -eq "review") -and $Specialist -ne "alia") {
  if ($PSBoundParameters.ContainsKey('Tokens') -and $Tokens -lt 0) {
    Write-Host ("[ERRO] Task " + $Status + " de '" + $Specialist + "' com -Tokens negativo de proposito: custo e obrigatorio no fecha. Omita -Tokens/-ToolUses pra medir automatico do transcript, ou informe os dois valores.")
    exit 1
  }
  if ($Tokens -lt 0 -or $ToolUses -lt 0) {
    $auto = Get-TranscriptUsage -SessionId $SessionId
    if ($auto.source -eq "auto-transcript") {
      if ($Tokens -lt 0) { $Tokens = $auto.tokens }
      if ($ToolUses -lt 0) { $ToolUses = $auto.tool_uses }
      $tokensSource = "auto-transcript"
    } else {
      $tokensSource = "indisponivel"
    }
  }
}
$budgetExceeded = $false
...
Add-Event -EventsFile $eventsFile -TaskId $newId -Step "REGISTRA" -Type "started" -Ref $Title
if (-not [string]::IsNullOrWhiteSpace($GateVerdict)) {
  Add-Event -EventsFile $eventsFile -TaskId $newId -Step "MONITORA" -Type "gate" -Ref $GateVerdict
}
if ($Status -eq "done") {
  Add-Event -EventsFile $eventsFile -TaskId $newId -Step "FECHA" -Type "done" -Ref $(if (-not [string]::IsNullOrWhiteSpace($Artifact)) { $Artifact } else { $newId })
} elseif ($Status -ne "open") {
  Add-Event -EventsFile $eventsFile -TaskId $newId -Step "DELEGA" -Type "delegated" -Ref $Status
}
exit 0
