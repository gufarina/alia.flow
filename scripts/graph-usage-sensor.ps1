...
try {
  # CEO, TASK-559 (correcao pos-Gate): forca UTF-8 sem BOM no stdout ANTES de qualquer
  # Write-Output - PowerShell escreve na codepage do console por padrao, corrompendo acento (o
  # sensor tambem imprime o deny do read-shunt-guard.ps1, entao precisa do mesmo forcing).
  ...
  # TASK-559 (item 3, gate do Nexus): guarda o -LedgerPath ORIGINAL (antes do default abaixo
  # preencher) - so quando o CALLER passou um caminho explicito (fixture de teste) isso viaja pro
  # guard de leitura; em producao fica vazio e o guard usa o proprio default (studio/read-shunt-log.jsonl,
  # nunca o graph-usage-log.jsonl deste sensor).
  $LedgerPathOriginal = $LedgerPath
  if ([string]::IsNullOrWhiteSpace($LedgerPath)) {
    $LedgerPath = Join-Path (Join-Path $root "studio") "graph-usage-log.jsonl"
  }

  # (0b) TASK-569 (L64): teto de chamadas por especialista, ANTES de qualquer classificacao -
  # dot-source preserva/restaura $root/$LedgerPath (mesmo clobber do read-shunt-guard acima,
  # budget-gate.ps1 tambem declara os dois parametros).
  try {
    ...
    $budgetResult = Invoke-BudgetGate -RawInput $raw -Root $root
    if (-not [string]::IsNullOrWhiteSpace($budgetResult)) { Write-Output $budgetResult; exit 0 }
  } catch { }
  ...
  # (1b) TASK-559: read-shunt-guard.ps1 cobre Read no proprio matcher (.claude/settings.json);
  # aqui, so Bash/PowerShell - 1 spawn por evento (regra da casa), dot-source em vez de novo hook.
  # $isShellTool ainda nao existe nesta altura do arquivo (so nasce no bloco (2) abaixo) - checagem
  # direta em $tool de proposito, pra nao inverter a ordem das secoes existentes.
  if ($tool -eq 'Bash' -or $tool -eq 'PowerShell') {
    try {
      # MEDIDO (WARDEN, TASK-559): dot-source reexecuta o param() de read-shunt-guard.ps1 na
      # NOSSA propria scope - como ele tambem declara $Root/$LedgerPath (PowerShell e case-
      # insensitive: $Root == $root), o dot-source sem argumentos ZERA $root e $LedgerPath deste
      # sensor pros defaults do outro script, quebrando toda classificacao/ledger daqui pra baixo
      # (prova: payload PowerShell isolado saia mudo, sem linha nenhuma no ledger). Preserva e
      # restaura em volta do dot-source pra neutralizar o clobber.
      $rsPreservedRoot = $root
      $rsPreservedLedgerPath = $LedgerPath
      . (Join-Path $PSScriptRoot "read-shunt-guard.ps1")
      $root = $rsPreservedRoot
      $LedgerPath = $rsPreservedLedgerPath
      $shuntResult = Invoke-ReadShuntGuard -RawInput $raw -Root $root -LedgerPath $LedgerPathOriginal
      if (-not [string]::IsNullOrWhiteSpace($shuntResult)) { Write-Output $shuntResult; exit 0 }
    } catch { }
  }
  ...
  # SO RODA script da propria casa (scripts/*.ps1 deste studio - raiz da instancia do operador/scripts/
 ...
 break
  ...
  # zero campo novo no schema - so um novo valor de $match).
  #
  # TASK-586 (15/09/2026): o teto de 2.500 BYTES cru cortava a string pela POSICAO no arquivo,
  # nao pela importancia - medido contra a secao real de God Nodes+Community Hubs: motor perdia
  # 66% (7.493 bytes), o maior mapa do parque perdia 97% (85.827 bytes, mapa com 22.363 nos) - o que
  # chegava ao especialista era so o que coube antes do byte 2.500 acabar, quase sempre so um
  # pedaco da lista de Community Hubs em ordem de indice, nunca os clusters mais centrais.
  # CONSERTO: seleciona por GRAU DE ENTRADA (nunca PageRank - fora de escopo, sem retorno medido
  # neste N), item inteiro por vez, orcamento em TOKENS (nao byte cru):
  #   - God Nodes: o proprio graphify ja imprime o grau ("N edges") por linha - so reordena por
  #     esse numero, nunca confia na ordem em que a linha caiu no arquivo.
  #   - Community Hubs: a secao "(Navigation)" e so um link, sem peso nenhum. Quando o mapa tem
  #     "## Communities" com "Nodes (N)" por cluster, usa esse tamanho (proxy de centralidade de
  #     cluster) pra ordenar os MESMOS hubs. Mapa sem "## Communities" (fixture pequena, mapa
  #     velho) mantem a ordem original - nunca inventa peso que o mapa nao deu.
  # O bloco injetado agora DECLARA data do mapa e cobertura (quantos de quantos itens entraram,
  # e quanto isso representa do total de nos do mapa) - um pedaco sem essas duas informacoes e a
  # armadilha conhecida: o especialista trata a fatia como se fosse o mapa inteiro.
  $script:INJECT_TOKENS_ALVO = 1000   # orcamento alvo do bloco injetado (piso ate aqui, sem pressa de parar)
  $script:INJECT_TOKENS_PISO = 400    # abaixo disto o bloco vira ruido - nunca corta antes
  $script:INJECT_TOKENS_TETO = 1600   # nunca injeta mais que isto - pesa o turno
  $script:INJECT_CHARS_POR_TOKEN = 4  # heuristica ascii/pt-BR (~4 caracteres por token)

  function Get-MapInjection([string]$MapFile) {
    try { $txt = [System.IO.File]::ReadAllText($MapFile) } catch { return $null }

    # Data do mapa: extrai do titulo "# Graph Report - <caminho> (YYYY-MM-DD)"; sem casar, cai
    # no LastWriteTime do arquivo - a injecao NUNCA sai sem data.
    $mapDate = $null
    $mTitle = [regex]::Match($txt, '\((\d{4}-\d{2}-\d{2})\)')
    if ($mTitle.Success) { $mapDate = $mTitle.Groups[1].Value }
    else {
      try { $mapDate = (Get-Item -LiteralPath $MapFile).LastWriteTime.ToString('yyyy-MM-dd') } catch { $mapDate = 'data desconhecida' }
    }

    $totalNodes = 0
    $mSummary = [regex]::Match($txt, '(\d+)\s+nodes')
    if ($mSummary.Success) { [void][int]::TryParse($mSummary.Groups[1].Value, [ref]$totalNodes) }

    # (a) God Nodes - reordena por "- N edges" quando o formato bate; formato diferente
    # (fixture/mapa antigo) cai no texto cru da secao, sem reordenar e sem descartar nada.
    $mGod = [regex]::Match($txt, [regex]::Escape('## God Nodes') + '.*?(?=\r?\n## |\z)', 'Singleline')
    $godHeader = $null
    $godRanked = New-Object System.Collections.Generic.List[object]
    $godRaw = $null
    if ($mGod.Success) {
      $godLines = ($mGod.Value.Trim() -split "`r?`n")
      $godHeader = $godLines[0]
      foreach ($ln in ($godLines | Select-Object -Skip 1)) {
        if ([string]::IsNullOrWhiteSpace($ln)) { continue }
        $mLn = [regex]::Match($ln, '-\s+(\d+)\s+edges\s*$')
        if (-not $mLn.Success) { $godRanked.Clear(); $godRaw = (($godLines | Select-Object -Skip 1) -join "`n"); break }
        $godRanked.Add([pscustomobject]@{ Line = $ln.Trim(); Degree = [int]$mLn.Groups[1].Value })
      }
    }

    # (b) Community Hubs - tamanho de cada cluster (se a secao "## Communities" existir) vira o
    # criterio de ordenacao dos mesmos links da secao "(Navigation)".
    $sizeByTitle = @{}
    $mComm = [regex]::Match($txt, [regex]::Escape('## Communities') + '.*?(?=\r?\n## |\z)', 'Singleline')
    if ($mComm.Success) {
      $reBlock = '### Community \d+ - "(.*?)"\r?\nCohesion:\s*[\d.]+\r?\nNodes \((\d+)\)'
      foreach ($mB in [regex]::Matches($mComm.Value, $reBlock)) { $sizeByTitle[$mB.Groups[1].Value] = [int]$mB.Groups[2].Value }
    }

    $mHub = [regex]::Match($txt, [regex]::Escape('## Community Hubs') + '.*?(?=\r?\n## |\z)', 'Singleline')
    $hubHeader = $null
    $hubRanked = New-Object System.Collections.Generic.List[object]
    if ($mHub.Success) {
      $hubLines = ($mHub.Value.Trim() -split "`r?`n")
      $hubHeader = $hubLines[0]
      $idx = 0
      foreach ($ln in ($hubLines | Select-Object -Skip 1)) {
        if ([string]::IsNullOrWhiteSpace($ln)) { continue }
        $size = -1
        $mName = [regex]::Match($ln, '\|(.*?)\]\]')
        if ($mName.Success -and $sizeByTitle.ContainsKey($mName.Groups[1].Value)) { $size = $sizeByTitle[$mName.Groups[1].Value] }
        $hubRanked.Add([pscustomobject]@{ Line = $ln.Trim(); Size = $size; Ordem = $idx })
        $idx++
      }
    }
    $hubSorted = @($hubRanked | Sort-Object -Property @{Expression = 'Size'; Descending = $true }, @{Expression = 'Ordem'; Descending = $false })

    # (c) preenche o orcamento (em bytes, convertido do alvo/piso/teto em tokens) do maior grau
    # pro menor, um item inteiro de cada vez - nunca corta item no meio.
    $byteAlvo = $script:INJECT_TOKENS_ALVO * $script:INJECT_CHARS_POR_TOKEN
    $bytePiso = $script:INJECT_TOKENS_PISO * $script:INJECT_CHARS_POR_TOKEN
    $byteTeto = $script:INJECT_TOKENS_TETO * $script:INJECT_CHARS_POR_TOKEN

    $sb = New-Object System.Text.StringBuilder
    $usado = 0
    $godIncl = 0
    $godTotal = 0
    if ($null -ne $godHeader) {
      [void]$sb.AppendLine($godHeader)
      if ($godRanked.Count -gt 0) {
        $ordered = @($godRanked | Sort-Object -Property Degree -Descending)
        $godTotal = $ordered.Count
        foreach ($g in $ordered) {
          if (($usado + $g.Line.Length) -gt $byteTeto) { break }
          [void]$sb.AppendLine($g.Line)
          $usado += $g.Line.Length
          $godIncl++
        }
      } elseif ($null -ne $godRaw) {
        [void]$sb.AppendLine($godRaw)
        $usado += $godRaw.Length
        $godIncl = 1; $godTotal = 1
      }
      [void]$sb.AppendLine('')
    }

    $hubIncl = 0
    $hubTotal = $hubSorted.Count
    $nosCobertos = 0
    if ($null -ne $hubHeader) {
      [void]$sb.AppendLine($hubHeader)
      foreach ($h in $hubSorted) {
        if ($usado -ge $byteAlvo -and $usado -ge $bytePiso) { break }
        if (($usado + $h.Line.Length) -gt $byteTeto) { break }
        [void]$sb.AppendLine($h.Line)
        $usado += $h.Line.Length
        $hubIncl++
        if ($h.Size -gt 0) { $nosCobertos += $h.Size }
      }
    }

    if ($godIncl -eq 0 -and $hubIncl -eq 0) { return $null }

    $coberturaTxt = $hubIncl.ToString() + '/' + $hubTotal + ' community hubs'
    if ($totalNodes -gt 0 -and $nosCobertos -gt 0) {
      $coberturaPct = [Math]::Round((100.0 * $nosCobertos / $totalNodes), 1)
      $coberturaTxt += ' (~' + $coberturaPct + '% dos ' + $totalNodes + ' nos do mapa)'
    }
    $coberturaTxt += ', ' + $godIncl.ToString() + '/' + $godTotal + ' god nodes'

    $header = '[FATIA DO MAPA] gerado em ' + $mapDate + ' | selecao por grau de entrada (nao por posicao no arquivo) | cobertura: ' + $coberturaTxt + ' - isto NAO e o mapa inteiro.'

    return $header + "`n`n" + $sb.ToString().TrimEnd()
  }
