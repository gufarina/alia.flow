...
# CONSERTO (TASK-589, GAUGE, 2026-09-15): graph-usage-sensor.ps1 mudou a classificacao de scan em
# 09/09/2026 (era substring "grep" em qualquer lugar do comando; virou 1o token do pipeline + alvo
# = diretorio, nao arquivo unico). O ledger NAO grava o comando bruto (so tool/kind/scope/match) -
# reclassificacao retroativa FIEL de linha antiga e IMPOSSIVEL. Exclui da conta (nem a favor nem
# contra) os scan de Bash/PowerShell gravados ANTES do conserto - direto na fonte, antes de
# qualquer numero derivado, para existir SO UM calculo (ate 2026-09-15 havia dois: o "oficial"
# ainda com o ruido antigo, imprimindo 27,4%, e uma linha "RECLASSIFICACAO" separada, calculada a
# mao pelo WARDEN em 09/09, mostrando o numero certo, 56%, sem nunca virar o veredito de verdade).
$SENSOR_FIX_UTC = [datetime]::Parse("2026-09-09T00:00:00.000Z", $inv, $styles)
$legacyUnknownCount = (@($allEvents | Where-Object { $_.kind -eq 'scan' -and ($_.tool -eq 'Bash' -or $_.tool -eq 'PowerShell') -and $_.ts -lt $SENSOR_FIX_UTC })).Count
$allEvents = New-Object System.Collections.Generic.List[object](, @($allEvents | Where-Object { -not ($_.kind -eq 'scan' -and ($_.tool -eq 'Bash' -or $_.tool -eq 'PowerShell') -and $_.ts -lt $SENSOR_FIX_UTC) }))
...
# a instancia do operador, Client sem squad como "clients/brax") - ler o mapa ali e IMPOSSIVEL, entao esses
...
Write-Host ("[OK] eventos no ledger: " + $totalLines + " | total lido (ja com o conserto de" +
  " classificacao TASK-589 aplicado): " + $allEvents.Count + " (mapa: " + $nMap + " | varredura: " +
  $nScan + ") | excluidos por classificacao antiga ambigua (scan Bash/PowerShell antes de 09/09): " +
  $legacyUnknownCount)
