...
try {
 ...
 # TASK-569 (L64): teto de chamadas por especialista - dot-source preserva/restaura $root
 # (mesmo padrao de graph-usage-sensor.ps1 com read-shunt-guard.ps1, pois budget-gate.ps1
 # tambem declara $Root/$LedgerPath e PowerShell e case-insensitive).
 $bgPreservedRoot = $root
 . (Join-Path $PSScriptRoot "budget-gate.ps1")
 $root = $bgPreservedRoot
 $budgetOut = Invoke-BudgetGate -RawInput $raw -Root $root
 if (-not [string]::IsNullOrWhiteSpace($budgetOut)) {
 Write-Output $budgetOut
 exit 0
 }

 # TASK-571 (L66): cerca do leitor barato - decide ANTES do delegation-gate, so dentro de
 # sub-agente. Dot-source preserva/restaura $root pelo mesmo motivo do budget-gate acima.
 $lgPreservedRoot = $root
 . (Join-Path $PSScriptRoot "leitor-gate.ps1")
 $root = $lgPreservedRoot
 $leitorOut = Invoke-LeitorGate -RawInput $raw -Root $root
 if (-not [string]::IsNullOrWhiteSpace($leitorOut)) {
 Write-Output $leitorOut
 exit 0
 }
