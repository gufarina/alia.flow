...
# Nome do studio pro titulo do HTML (-Html): SEMPRE em runtime, via alia.config.json (campo
# "studio") DESTA instalacao - nunca hardcoded. Instalacao sem alia.config.json/campo -> fallback
# neutro "Alia Flow" (nome do motor, nao de operador nenhum).
$studioName = "Alia Flow"
$aliaCfgPath = Join-Path $studioRoot "alia.config.json"
if (Test-Path -LiteralPath $aliaCfgPath) {
  try {
    $aliaCfg = [System.IO.File]::ReadAllText($aliaCfgPath) | ConvertFrom-Json
    $v = "$($aliaCfg.studio)".Trim()
    if ($v -ne "") { $studioName = $v }
  } catch {}
}
...
<title>Mapa de linhagem do ledger - __STUDIO_NAME__</title>
<style>
  :root{
...
<body>
'@
  $head = $head.Replace('__STUDIO_NAME__', [System.Net.WebUtility]::HtmlEncode($studioName))
