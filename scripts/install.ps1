...
try {
  ...
  # Remove-Item direto lancava "An object at the specified path ... does not exist" nesse
  # caso, e a limpeza de temporario NUNCA pode derrubar o instalador com exit 1. Resolve o
  # caminho completo (quando o item ainda existe) e apaga cada um com -LiteralPath, isolado
  # em try/catch - erro de limpeza fica silencioso, nunca propaga.
  foreach ($tmpItem in @($tmpZip, $tmpDir)) {
    try {
      if (-not (Test-Path -LiteralPath $tmpItem)) { continue }
      $fullItem = $tmpItem
      try { $fullItem = (Get-Item -LiteralPath $tmpItem -Force).FullName } catch { }
      Remove-Item -LiteralPath $fullItem -Recurse -Force -ErrorAction Stop
    } catch { }
  }
}
