...
foreach ($clientDir in $clientDirs) {
 ...
 throw "falta 'authority.escalates_to' no yaml (obrigatorio em toda camada - a quem sobe e em que caso). Sugestao para camada $camada`: $($defAuthority.escalates_to)"
            ...
            # TASK-571 (L66): camada B/C ganha Task na lista SOMENTE porque leitor-gate.ps1
            # (PreToolUse) cerca esse Task a um unico uso permitido - o leitor em massa
            # (Explore/haiku/molde skills/leitura-em-massa/SKILL.md). Sem a cerca ligada, isto
            # seria fan-out sem teto; com ela, Task de folha e sempre deny fora do molde.
            if (-not $validTools -or $validTools.Count -eq 0) {
                switch ($camada) {
                    'A' { $validTools = @('Read', 'Grep', 'Glob', 'Task') }
                    'B' { $validTools = @('Read', 'Grep', 'Glob', 'Edit', 'Write', 'Bash', 'Task') }
                    default { $validTools = @('Read', 'Grep', 'Glob', 'Task') }
                }

            }

            elseif ($validTools -notcontains 'Task') {
