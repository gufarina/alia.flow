<#
  ...
  vX.Y.Z: o que mudou em ate 6 palavras. Sem nome, cargo, mandato, lei, data ou rodape.
#>
param([switch]$Force)
...
# TASK-624 (WARDEN/CEO, 17/09/2026): piso do CHANGELOG publico. O CHANGELOG.md da oficina
# carrega o historico inteiro (nome de instancia, TASK-N, friction-*) que nunca pode viajar
# pro produto publico. Constante nomeada e comentada aqui de proposito: quem for mudar o corte
# daqui a um ano acha em 5 segundos, sem escavar a logica de truncagem abaixo.
$ChangelogPublicFloor = "1.79.0"
...
# benchmarks/ NAO entra no pacote (TASK-624, decisao do CEO 17/09/2026, reverte o TASK-015):
# sao 7 scripts Python soltos num produto PowerShell-first - medicao interna da oficina, sem
# nenhum script do produto que viaja invocando-os em runtime. Ficam na oficina, fora do ship.
...
foreach ($d in $shipDirs) {
  ...
  if (Test-Path $src) {
    ...
    # engine,docs,skills -Recurse -EdgesOnly, regenerado a cada rodada do smoke. Empacotar levaria
    # uma FOTO CONGELADA do lab que fica podre no instante em que a instancia de destino diverge -
    # o mesmo defeito que a deteccao de mapa velho existe para evitar. O que viaja e o GERADOR
    # (kb-index.ps1, ja na allowlist de scripts/); cada instancia gera o seu proprio.
    if ($d -eq "engine") { $xf += (Join-Path $src "governance\edges.json") }
    # TASK-615: capability-ledger.md e secrets.md sao diario de manutencao do motor, prosa
...
# INSTANCIA aplicada do operador, nao do motor - para todo pacote publico). Allowlist falha fechado:
...
$scriptsAllow = @(
  "_studio.ps1","_manifest-exclude.ps1","alinhamento-gate.ps1","budget-check.ps1","check-public-surface.ps1","client-state.ps1","cost-per-artifact.ps1","cost-sensor.ps1","ddd-drift.ps1",
  "debt-scan.ps1","delegation-gate.ps1","delegation-guard.ps1","desperdicio.ps1","detect-harness.ps1","docs-check.ps1","doctor.ps1","evolution-scan.ps1","git-sync.ps1",
  "gate-check.ps1","graph-check.ps1","graph-usage-sensor.ps1","graph-usage.ps1","guard-core.ps1","health-check.ps1",
...
# TASK-624: CHANGELOG.md truncado na publicacao, nunca copiado inteiro. Ancora e o cabecalho
# de versao "## [X.Y.Z] - AAAA-MM-DD": tudo ACIMA da primeira entrada (titulo, "Keep a
# Changelog", "Esquema de versao") e contrato util ao usuario e fica; da entrada de
# $ChangelogPublicFloor pra cima fica; da entrada ANTERIOR ao floor em diante sai (e e
# exatamente onde o nome da instancia e o TASK-N moram). Falha fechado: sem a entrada do
# floor no CHANGELOG, aborta - nunca publica o historico inteiro por omissao silenciosa.
$changelogOutPath = Join-Path $out "CHANGELOG.md"
if (Test-Path -LiteralPath $changelogOutPath) {
  $clLines = Get-Content -LiteralPath $changelogOutPath -Encoding UTF8
  $floorPattern = '^## \[' + [regex]::Escape($ChangelogPublicFloor) + '\]'
  $floorIdx = -1
  for ($i = 0; $i -lt $clLines.Count; $i++) { if ($clLines[$i] -match $floorPattern) { $floorIdx = $i; break } }
  if ($floorIdx -lt 0) {
    Write-Host ("[ERRO] CHANGELOG.md nao tem entrada de [" + $ChangelogPublicFloor + "] - corte de publicacao abortado.")
    exit 1
  }
  $cutIdx = $clLines.Count
  for ($i = $floorIdx + 1; $i -lt $clLines.Count; $i++) { if ($clLines[$i] -match '^## \[') { $cutIdx = $i; break } }
  $kept = $clLines[0..($cutIdx - 1)]
  $kept += ""
  $kept += ("Historico anterior a " + $ChangelogPublicFloor + " nao e publicado neste repositorio.")
  [System.IO.File]::WriteAllText($changelogOutPath, (($kept -join "`n") + "`n"), (New-Object System.Text.UTF8Encoding($false)))
  Write-Host ("    CHANGELOG.md truncado na publicacao: a partir de [" + $ChangelogPublicFloor + "] (" + $kept.Count + " linhas).")
}
...
# docs/assets/ e allowlist de ARQUIVO, mesmo criterio do bloco acima: so o que o README
# publicado referencia. Hoje so olho-alia.gif (README, linha 1).
$docsAssetsAllow = @("olho-alia.gif")
$docsAssetsSrc = Join-Path $docsSrc "assets"
if (Test-Path $docsAssetsSrc) {
  foreach ($af in $docsAssetsAllow) {
    $asrc = Join-Path $docsAssetsSrc $af
    if (Test-Path -LiteralPath $asrc) {
      New-Item -ItemType Directory -Force -Path (Join-Path $out "docs\assets") | Out-Null
      Copy-Item -LiteralPath $asrc -Destination (Join-Path $out ("docs\assets\" + $af)) -Force
      Write-Host ("    [file] docs\assets\" + $af)
    }
  }
}
...
# que empacota). studio/ e dado de operador/runtime, nunca deveria entrar no manifesto nem no
# pacote publicado - senao a integridade falha em QUALQUER maquina diferente da que empacotou
# (o hash do baseline muda por host). Apaga antes do manifesto, defesa em profundidade da
# mesma regra da linha 216.
$outStudio = Join-Path $out "studio"
if (Test-Path -LiteralPath $outStudio) {
  Remove-Item -Recurse -Force $outStudio
  Write-Host "    studio/ (efeito colateral do smoke dentro do pacote) removido antes do manifesto."
}

# TASK-597: mesmo defeito do studio/ acima - a exclusao de engine/governance/edges.json (linha ~113)
# roda ANTES do smoke, mas o smoke DENTRO do pacote (linha ~270) o REGENERA como efeito colateral
# (kb-index -EdgesOnly, TASK-587/588/590). Exclusao antes do smoke nao basta; mesmo tratamento
# pos-smoke que studio/ ja recebe.
$outEdges = Join-Path $out "engine\governance\edges.json"
if (Test-Path -LiteralPath $outEdges) {
  Remove-Item -Force $outEdges
  Write-Host "    engine\governance\edges.json (regenerado pelo smoke dentro do pacote) removido antes do manifesto."
}
...
# Confere na hora (TASK-567): gerar sem verificar deixa passar um manifesto que ja nasce
# divergente do pacote (ex.: exclusao assimetrica entre make e verify). Aborta o empacotamento
# se o proprio pacote recem-gerado nao bater com o manifesto que acabou de sair dele.
& (Join-Path $PSScriptRoot "verify-manifest.ps1") -Dir $out | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Host "[ERRO] o manifesto de integridade recem-gerado nao bate com o pacote. Empacotamento abortado."; exit 1 }
Write-Host "manifesto de integridade verificado contra o pacote: OK."
